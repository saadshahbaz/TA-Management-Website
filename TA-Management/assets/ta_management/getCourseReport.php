...
$sql = $conn->prepare(
    'SELECT DISTINCT email,student_id,ta_name FROM TA WHERE course=:course AND term=:term AND years=:years'
);
...
while ($ta = $result->fetchArray()) {
    ...
    $sql2->bindValue(':studentID', $ta['student_id']);
    ...
    $sql3 = $conn->prepare(
        'SELECT COUNT(course) as total_courses FROM `TA` WHERE student_id = :studentID and email = :email'
    );

    // $sql3->bind_param('ss', $ta['student_id'], $ta['email']);
    $sql3->bindValue(':studentID', $ta['student_id']);
    $sql3->bindValue(':email', $ta['email']);
