...
function getMessages($year, $term, $course)
{
    ...
    global $NotFound;
    $sql = $conn->prepare(
        'SELECT * FROM message WHERE course= :course AND term= :term AND year= :years ORDER BY time DESC'
    );
    ...
    while ($oh = $result->fetchArray()) {
        ...
        $sql2->bindValue(':email', $oh['user']);
        $result2 = $sql2->execute();
        // $result2 = $sql2->get_result();
        $user = $result2->fetchArray();

        echo '<h4 style="color: rgb(167, 37, 48);">Posted By: ' .
            $user['firstName'] .
            ' ' .
            $user['lastName'] .
            '</h4>';
